...
it("advertises the Claude Code version this install actually ships", () => {
  // Read from the constant rather than pinned to a literal. Pinned, it went
  // stale the moment the constant moved and stayed red for two releases —
  // which is the opposite of what a version assertion is for.
  const body = applyCloaking({ messages: [] }, "sk-ant-oat-test", "session-id");
  expect(body.system[0].text)
    .toMatch(new RegExp(`^x-anthropic-billing-header: cc_version=${CLAUDE_CLI_VERSION.replace(/\./g, "\.")}\.`));
});
