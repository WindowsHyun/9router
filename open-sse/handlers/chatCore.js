...
import { injectAgentSkills } from "../skills/agentSkills.js";
...
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, rtkEnabled, headroomEnabled, headroomUrl, headroomCompressUserMessages, headroomTimeoutMs, cavemanEnabled, cavemanLevel, ponytailEnabled, ponytailLevel, agentSkills, pxpipeEnabled, pxpipeMinChars, pxpipeTimeoutMs, pxpipeTransform, onPxpipeEvent, sourceFormatOverride, providerThinking }) {
  ...
  // Agent skills: append each enabled SKILL.md to the system prompt. Not gated
  // on tokenSaverEnabled — a skill changes how the model answers, which is not
  // a compression setting, and it costs tokens rather than saving them.
  const injectedSkills = injectAgentSkills(translatedBody, finalFormat, agentSkills);
  if (injectedSkills.length > 0) xf.push(`SKILLS:${injectedSkills.join(",")}`);
